#ifndef PHEROES_GAME_BATTLEUNIT_H_
#define PHEROES_GAME_BATTLEUNIT_H_
...
public:
	...
	inline const iPoint& ScreenPos() const { return m_ScreenPos; }
	...
	void GoodLuck();
	void BadLuck();
	uint32 CalcJoustingBonus(iPoint tgtCell) const;
	...
	void AttackEstimate(iBattleGroup* pTarget, uint32 aflags, sint32 penalty, uint32 joustBonus, MORLUCK_MOD luck, bool maxDamage, sint32& damage, sint32& deaths);
	sint32 ReceiveDamage(sint32& damage, bool bRange);
	sint32 ReceiveDamageEstimate(sint32& damage, bool bRange);
	sint32 Half(sint32& damage);
	bool CanResurrect() const;
	sint32 CountResurrected(sint32 hp);
	sint32 Resurrect(sint32 hp);
	void Restore(sint32 hp);
	void SetState(STATE nState);
	void Place(const iPoint& pos);
	void ApplyBonusList(const iCreatBonusList &cbl);
	inline void SetFrame(const sint32 frame) { m_frame = frame; }
	inline sint32 Frame() const { return m_frame; }
	inline void ScreenPlace(const iPoint& pos) { m_ScreenPos = pos; }
	void Rotate();
    ...
    inline void ActivatePoison(uint32 dmg) { m_poison_damage = dmg; m_poison_counter = 3; }
	inline uint32 GetPoisonDuration() const { return m_poison_counter; }

protected:
	iFurtSkills		m_furtSkills;

private:
    uint32          m_poison_damage;
    uint32          m_poison_counter;
...
public:
	...
	iBattleUnit_Catapult(iBattleMember* pOwner) : iBattleUnit(Catapult, pOwner), m_state(Idle) {}
	bool IsAlive() const { return true; }
	bool CanWait() const { return false; }
	sint32 Speed() const { return 0xFF; }
	CatapultState& State() { return m_state; }
private:
	CatapultState m_state;
};
//
...
#endif //PHEROES_GAME_BATTLEUNIT_H_
