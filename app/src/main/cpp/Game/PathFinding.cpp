...
void iPath::DumpPath() const
{
	...
	for (uint32 xx=0; xx<m_Path.GetSize(); ++xx)
    {
        ...
        {
            sint32 idx = 4 + iSIGN(m_Path[xx+1].m_Pos.x - m_Path[xx].m_Pos.x )*3 + iSIGN( m_Path[xx+1].m_Pos.y - m_Path[xx].m_Pos.y );
            static const uint8 dirs[9] = { 0, 7, 6, 1, 0, 5, 2, 3, 4 };
            angle = dirs[idx];
        }
		debugf(L"--- Step (%d,%d) : %d [%d] A%d\n", m_Path[xx].m_Pos.x, m_Path[xx].m_Pos.y, m_Path[xx].m_Cost, m_Path[xx].m_action, angle);
	}
...
sint32 iPath::GetStepCostTillValid() const
{
	sint32 res = 0;
	uint32 xx=0;
	if (m_Path.GetSize() <= 0)
...
void iPath::ProcessShipOperations( sint32 initialMoves )
{
...
uint16 GetMoveCost(uint16 pm, uint8 dir) {
	return CalculateCost(pm, dtab[dir].coef);
}

iPoint GetDirOffset(uint8 dir)
{
	return iPoint(nOffs[dir][0], nOffs[dir][1]);
}
...
sint32 iPathFinder::FindPath(const iPoint& srcPos, const iPoint& dstPos, iPath& path, uint8 spMask, uint8 epMask, bool is_water)
{
	TSTART(findpath);
...
//            if (is_water)
//                wcost = passMap[ npt.Idx(mapW) ] == PASSMAP_SWATER ? 0 : -1;
//            else
...
sint32 iPathFinder::FindPathToShip(const iPoint& srcPos, const iPoint& dstPos, iPath& path, uint8 spMask, uint8 epMask)
{
    // Cotulla, main ideas:
    // 1. reverse path find direction (from end to start)
    // 2. add special "ground" flag
    // 3. on ground -> water switch reset flag to 0, so it won't allow ground paths more
    // 4. fill path in reversed order, on water -> ground switch put Leave instead of move
    TSTART(findpath);
    SmallPnt startp((uint8)dstPos.x, (uint8)dstPos.y );
    SmallPnt endp( (uint8)srcPos.x, (uint8)srcPos.y );
    DOUT( L"# routing from %d,%d to %d,%d\n", srcPos.x, srcPos.y, dstPos.x, dstPos.y );
    ...
    // enabled direction
    SetupEntryExitFlags( startp.yx, endp.yx, epMask, spMask);
    ...
    while ( lastHeapNode != 0 )
    {
        ...
        for( size_t dirCode = 0; dirCode != 8; ++dirCode )
        {
            ...
            if ( wpnt.state == SUnknown )
            {
                ...
                newNode.yx	= npt.yx;
                newNode.g	= newG;
                uint32 newNodef	= newG + Distance( npt, srcPos.x, srcPos.y );
...
//////////////////////////////////////////////////////////////////////////
// this function used only for ships, have special conditions for generated path
//////////////////////////////////////////////////////////////////////////

sint32 iPathFinder::FindPathForShip(const iPoint& srcPos, const iPoint& dstPos, iPath& path, uint8 spMask, uint8 epMask)
{
    // Cotulla, main ideas:
    // 1. reverse path find direction (from end to start)
    // 2. add special "ground" flag
    ...
    // init only water
    SmallPnt spt(startp.yx);
    sint8 pm = passMap[spt.Idx(mapW)];
    if (pm == PASSMAP_SWATER || pm == PASSMAP_PWATER)
        world[spt.Idx(mapW)].flag = true;
    else
    ...
    // enabled direction
    SetupEntryExitFlags( startp.yx, endp.yx, spMask, epMask); //epMask, spMask); Cotulla: fix for notaccessable objects from ship
    ...
    while ( lastHeapNode != 0 )
    {
        ...
        {
            world[ parentPt.Idx(mapW) ].flag = true;
        }
        p_onlywater = world[ parentPt.Idx(mapW) ].flag;
    ...
    if ( lastHeapNode != 0 ) {
        ...
        if ( is_good && dstNode.g < MAX_PATH_COST )
        {
            ...
            bool old_flag = world[cur.Idx(mapW)].flag;
            sint8 old_cost = passMap[cur.Idx(mapW)];
#ifdef _DEBUG
            ...
            while ( !(cur == startp) )
            {
...
#endif
                //leave_ship = (old_cost == PASSMAP_SWATER && cost != -128);
                old_cost = cost;
                ...
                // get next point .flag value
                flag = world[cur.Idx(mapW)].flag;
                //leave_ship = flag == false && old_flag != flag;
                leave_ship = old_flag == true && flag == false;
                old_flag = flag;
                path.AddMoveToPointShip(tpt.ToPoint(), cost, leave_ship);
            }
            ...
            //leave_ship = old_flag == true && flag == false;
            leave_ship = old_flag == true && flag == false;
            path.AddMoveToPointShip(cur.ToPoint(), cost, leave_ship);
            DOUT(L"### adding point %d,%d cost %d, dir %d\n", cur.x, cur.y, cost, dir);
...
, mapH( pmap.GetHeight() )
{
	...
	if ( maxNodesNum < TUNE_NODESINITIAL ) {
		...
		nodes = (Node*)ReallocPODArray( nodes, 0, sizeof(Node) * maxNodesNum );
	}
	nodes[0].yx = 0;
	nodes[0].g	= 0;
