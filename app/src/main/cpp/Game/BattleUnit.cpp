...
SPELL_DISPOS iBattleGroup::SpellsDispos() const
{
	...
	if(GetPoisonDuration() > 0) negative++;
...
void iBattleGroup::NewRound()
{
	...
	// fury perk
	// if still alive and remains less than 50% of initial count, put bloodlust 
	// for unlimited time
	
	if (CREAT_DESC[m_creatType].perks & CPERK_FURY && m_creatCount <= m_initCount / 2 && m_creatCount > 0)
	{                        
		iCombatSpell *cs = (iCombatSpell*)gGame.ItemMgr().m_spellMgr.Spell(MSP_BLOODLUST);
		sint32 duration = 999;
		sint32 param = SPELL_DESCRIPTORS[MSP_BLOODLUST].eff[MSL_EXPERT].sparam;
		if (AddSpellToList(this, cs, duration, param)) {
			FurtSkills().Value((FURTHER_SKILLS)SPELL_DESCRIPTORS[MSP_BLOODLUST].eff[MSL_NONE].fparam) += param;
		}
	}	
...
// 05.08.2010 new perk - poison support
    if (m_poison_counter > 0)
    {
        // calculate damage
        sint32 damage = m_poison_damage;
        ...
        if (m_poison_counter == 3)
            damage = damage / 4;
        else if (m_poison_counter == 2)
            damage = damage / 10;
        else if (m_poison_counter == 1)
            damage = damage / 20;
...
void iBattleGroup::GoodMorale()
{
	...
	m_pWrapper->AddCellEvent(iFormat(_T("#I%04X"), PDGG(ICN_MORALE)), m_Pos);
}
...
void iBattleGroup::BadMorale()
{
	...
	m_pWrapper->AddCellEvent(iFormat(_T("#I%04X"), PDGG(ICN_MORALE)+2), m_Pos);
}
...
void iBattleGroup::GoodLuck()
{
	...
	m_pWrapper->AddCellEvent(iFormat(_T("#I%04X"), PDGG(ICN_LUCK)), m_Pos);
}
...
void iBattleGroup::BadLuck()
{
	...
	m_pWrapper->AddCellEvent(iFormat(_T("#I%04X"), PDGG(ICN_LUCK)+2), m_Pos);
}

uint32 iBattleGroup::CalcJoustingBonus(iPoint tgtCell) const
{
	if (CREAT_DESC[m_creatType].perks & CPERK_JOUSTING) {
		if(DistMap().GetAt(tgtCell.x, tgtCell.y).dirs[0].val != 0xFF)
			return 5 * DistMap().GetAt(tgtCell.x, tgtCell.y).dirs[0].val;		
		else if(DistMap().GetAt(tgtCell.x, tgtCell.y).dirs[1].val != 0xFF)
			return 5 * DistMap().GetAt(tgtCell.x, tgtCell.y).dirs[1].val;		
	}
	return 0;
}
...
sint32 iBattleGroup::CalcDamage(const iBattleGroup* pTarget, sint32 damage, bool bRange, sint32 penalty, uint32 joustBonus) const
{
    ...
    {
        adif += m_furtSkills.Value(FSK_MELEEATTACK);		
    }
	...
	// Holy gives 2x bonus against undead enemies
    if (HasPerk(CPERK_HOLY) && pTarget->HasPerk(CPERK_UNDEAD) && damage > 0)
    {   
        damage *= 2;
    }
...
void iBattleGroup::Attack(iBattleGroup* pTarget, uint32 aflags, sint32 penalty, uint32 joustBonus, MORLUCK_MOD luck)
{
    ...
    //
	iHero * pHero = m_pOwner->SpellCaster();
	if (bRange && (aflags & LichCloud) == 0 && (!pHero || !pHero->Artifacts().HasArtType(ARTT_ARROWS))) {
//		check(m_shots > 0);
		if( m_shots > 0 )
		--m_shots;
	}
	...
	if (HasPerk(CPERK_DOHALF) && gGame.Map().Rand(10) == 0 &&
		damage < pTarget->TotalHitPoints() / 2) {
#ifdef MULTIPLAYER
		check if random gets synced
#endif
        ...
        // poison adds poison counter for 3 turns
        if (HasPerk(CPERK_POISON))
        {
            pTarget->ActivatePoison(damage);
        }

		// blind gives 20% chance of blinding the target
        if (HasPerk(CPERK_BLIND))
        {
		    iCombatSpell *cs = (iCombatSpell*)gGame.ItemMgr().m_spellMgr.Spell(MSP_BLIND);
            // 20% blind on attackers
            if (gGame.Map().Rand(100) <= 20 && cs->CanBeCastOn(pTarget, MSL_EXPERT, false))
            {                            
                sint32 duration = 3;
                AddSpellToList(pTarget, cs, duration, 0);
            }
        }

		// 100% chance of cursing the target
        if (HasPerk(CPERK_CURSE))
        {   
			iCombatSpell *cs = (iCombatSpell*)gGame.ItemMgr().m_spellMgr.Spell(MSP_CURSE);
			if(cs->CanBeCastOn(pTarget, MSL_EXPERT, false)) {
				sint32 duration = 3;
				AddSpellToList(pTarget, cs, duration, 0);
			}
        }
...
{    
	iStringT cellMsg = iStringT();
    
    bool bAdvCombat = gSettings.GetEntryValue(CET_ADVANCED_COMBAT)>0;
    
    if( bAdvCombat )
        cellMsg += FormatNumber(damage);
...
sint32 iBattleGroup::Resurrect(sint32 hp)
{
	...
	// after resurrection, at least 1 creature must be alive
	if(m_creatCount == 0) {
		resurrected = 1;
		m_creatCount = 1;
	}
	...
	if (m_State == Dead)  {
		SetState(Idle);
		m_Orient = (ORIENT)m_pOwner->GetSide();
	}
	return resurrected;
}
...
void iBattleGroup::ApplyBonusList(const iCreatBonusList &cbl)
{
	for(uint32 x=0; x < cbl.GetSize(); x++)
		switch(cbl[x].bt) {
			case CBT_ATTACK:
				FurtSkills().Value(FSK_ATTACK) += cbl[x].bonus;
				break;
			case CBT_DEFENCE:
				FurtSkills().Value(FSK_DEFENCE) += cbl[x].bonus;
				break;
			case CBT_SPEED:
				FurtSkills().Value(FSK_SPEED) += cbl[x].bonus;
				break;
...
void iBattleGroup::GetMeleePosition(const iBattleGroup* pTarget, const iPoint& pos, uint16 mdir, iPoint& meleePos, uint8& meleeOrient)
{
	if (mdir == MDIR_RSPTOP || mdir == MDIR_RSPBOTTOM || mdir == MDIR_LSPTOP  || mdir == MDIR_LSPBOTTOM) {
		...
		//} else {
		//	meleePos.x = pos.x + HEX_MELEE_NEBS2[pos.y&1][meleeOrient][dir][0];
		//	meleePos.y = pos.y + HEX_MELEE_NEBS2[pos.y&1][meleeOrient][dir][1];	
		//}
	}
}


void iBattleGroup::Synchronize(iArmyOwner* pOwner)
{
	if (m_idx == -1) return;
	const iArmy& army = pOwner->Army();
	//check(army[m_idx].Type() == m_creatType);
	pOwner->SetArmyGroupCount(m_idx, m_creatCount);
	if (m_casualties) {
		m_casualties = 0;
		check(army[m_idx].Count() >= 0);		
	}
	if (!army[m_idx].Count()) pOwner->SetArmyGroupType(m_idx, CREAT_UNKNOWN);
}
