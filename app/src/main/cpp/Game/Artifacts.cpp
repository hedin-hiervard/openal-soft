...
iArtCtr::iArtCtr(iHero* pOwner)
{
	...
	for (uint32 xx=0; xx<AC_COUNT; ++xx) 
		m_Dressed.Add(iArtCell((HERO_ART_CELL)xx));
}

void iArtCtr::Update(iHero* pOwner)
{
	m_pOwner = pOwner;
	m_Dressed.RemoveAll();
...
bool iArtCtr::HasCreatBonus(CREATURE_TYPE ct, iCreatBonusList &cbl) const
{
	bool res = false;
	for (uint32 xx=0; xx<m_Dressed.GetSize(); ++xx) {
		const iCreatBonus *crBonus;
		if (!m_Dressed[xx].Empty() && (crBonus = gGame.ItemMgr().m_artMgr[m_Dressed[xx].artId].CreatureBonus())) {
			if(crBonus->ct == ct) {
				cbl.Add(*crBonus);
				res = true;
			}
...
bool iArtCtr::HasMagicSphere(MAGIC_SCHOOL sch) const
{
	for (uint32 xx=0; xx<m_Dressed.GetSize(); ++xx) {
		if(!m_Dressed[xx].Empty() && gGame.ItemMgr().m_artMgr[m_Dressed[xx].artId].Type() == ARTT_MAGICSPHERE) {
			iArtT_MagicSphere* art = (iArtT_MagicSphere *)(&gGame.ItemMgr().m_artMgr[m_Dressed[xx].artId]);
			if (art && art->Type() == sch) return true;
		}
	}
	return false;
}
...
void iArtCtr::Push(uint16 artId)
{
	...
	gGame.CheckPlayerWinAndLose(m_pOwner->Owner());
}
...
bool iArtCtr::HasArtifact(uint16 artId) const {
	for(uint32 xx=0; xx<m_Dressed.GetSize(); xx++) {
		if(m_Dressed[xx].artId == artId)
			return true;	
	} 

	for(uint32 xx=0; xx<m_BackPack.GetSize(); xx++) {
		if(m_BackPack[xx] == artId)
			return true;
	}
...
sint32 iArtCtr::SmartPop(uint16 artId) {
	sint32 res = 0;
	for(uint32 xx=0; xx<m_Dressed.GetSize(); xx++) {
		if(m_Dressed[xx].artId == artId) {		
			m_Dressed[xx].Reset();
			gGame.ItemMgr().m_artMgr[artId].Dettach(m_pOwner);	
			res++;
		}
	} 
