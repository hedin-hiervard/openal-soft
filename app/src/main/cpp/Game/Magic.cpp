...
bool iSpell_TownPortal::Cast(iHero* pCaster)
{
	...
	if (pOwner->PlayerType() == PT_HUMAN) {		
		if (SPELL_DESCRIPTORS[m_spellId].eff[msl].fparam) {
			// Town portal
			iDlg_TownList tlDlg(&gApp.ViewMgr(), pOwner, gTextMgr[TRID_SELECT_DESTINATION], TRID_OK, true);
...
bool iCombatSpell::CanBeCastOn(iBattleGroup* pTarget, MAGIC_SCHOOL_LEVEL msl, bool bFriendly) const
{
...
bool iCombatSpell::CanCast(iBattleUnit_Hero* pCaster, iBattleGroup* pTarget) const
{
	...
	return CanBeCastOn(pTarget, msl, bFriendly);
}
...
//////////////////////////////////////////////////////////////////////////
sint32 iSpell_Damage::SpellBookDamage(iHero* pCaster) const
{
    ...
    // magic sphere bonus
    sint32 perc = 0;
    if(pCaster->Artifacts().HasMagicSphere((MAGIC_SCHOOL)SPELL_DESCRIPTORS[m_spellId].school))
        perc += 50;
    if (sint32 spelldmg = pCaster->FurtSkill(FSK_SPELLDMG)) 
        perc += spelldmg;
...
bool iSpell_Damage::Cast(iBattleUnit_Hero* pCaster, iBattleGroup* pTarget) const
{
	...
	iStringT cellMsg = iStringT();//(FormatNumber(damage));	
...
bool iSpellBook::HasOverlandSpells() const
{
	for(uint32 xx=0; xx<m_spells.GetSize(); xx++) {
		if(SPELL_DESCRIPTORS[m_spells[xx]].type == SPT_OVERLAND)
			return true;
	}
	return false;
}
