...
iTextComposer::FontConfig fc_hdr(FS_MEDIUM, iDibFont::ComposeProps(RGB16(255,255,0)));
iTextComposer::FontConfig fc_topic(FS_SMALL, iDibFont::ComposeProps(RGB16(160,192,255)));
iTextComposer::FontConfig fc_topic2(FS_SMALL, iDibFont::ComposeProps(RGB16(255,192,160)));
iTextComposer::FontConfig fc_plain(FS_SMALL, iDibFont::ComposeProps(RGB16(192,192,192),RGB16(32,32,32)));
iTextComposer::FontConfig fc_info(FS_SMALL, iDibFont::ComposeProps(RGB16(255,192,0),RGB16(0,0,0)));
...
void iPopupDlg_Castle::DoCompose(const iRect& clRect)
{
	...
	// Icon
//	SpriteId sid = PDGG(CTL_SICONS) + (m_pCastle->Proto()->Size()-1)*CTLT_COUNT + m_pCastle->Proto()->Type();
...
void iPopupDlg_Hero::DoCompose(const iRect& clRect)
{
	...
	for (sint32 psid=0; psid<PRSKILL_COUNT; ++psid) {
		if (m_vLevel==VL_EXPERT) psText.Addf(_T("#I%04X #FCCC%d "), PDGG(ICN_PSKILLS)+psid, m_pHero->FurtSkill((FURTHER_SKILLS)(FSK_ATTACK+psid)));
