...
one is the only random factor - damage lies somewhere w5ithin damage
...
//#define SLO_MO
//////////////////////////////////////////////////////////////////////////
iBattleAct_Move::iBattleAct_Move(iBattleGroup* pActor, const iPoint& target, sint32 frame) 
#ifndef SLO_MO
: iBattleAct(fix32(0.3f / 6)), m_pActor(pActor), m_target(target), m_bridgeFlag(0), m_frame(frame) {}
#else
: iBattleAct(fix32(0.5f)), m_pActor(pActor), m_target(target), m_bridgeFlag(0), m_frame(frame) {}
#endif
...
{ 
	...
	m_pActor->ScreenPlace(cInvalidPoint);
	m_pActor->SetState(iBattleGroup::Idle); 
	m_pActor->SetFrame(m_frame);


	/*iPoint t = Map2Screen(m_target);
	WCHAR buff[1024]={0};
	wsprintf(buff,L"FRAME (%d): %d %d\n", m_frame, t.x, t.y);
	OutputDebugString(buff);
	*/	
}
//////////////////////////////////////////////////////////////////////////

iBattleAct_Anim::iBattleAct_Anim(iBattleGroup* pActor, const iPoint& target, sint32 frame) 
#ifndef SLO_MO 
: iBattleAct(fix32(0.3f / 6)), m_pActor(pActor), m_target(target), m_frame(frame) {}
#else
: iBattleAct(fix32(0.5f)), m_pActor(pActor), m_target(target), m_frame(frame) {}
#endif 

void iBattleAct_Anim::BeginAct()
{ 
	m_pActor->SetState(iBattleGroup::Moving); 
	
}

void iBattleAct_Anim::EndAct()
{ 
	/*WCHAR buff[1024]={0};
	wsprintf(buff,L"FRAME (%d): %d %d\n", m_frame, m_target.x, m_target.y);
	OutputDebugString(buff);
	*/

	m_pActor->ScreenPlace(m_target);
	m_pActor->SetFrame(m_frame);	
}
...
iBattleAct_BeginMoving::iBattleAct_BeginMoving(iBattleGroup* pActor) 
: iBattleAct(fix32::zero), m_pActor(pActor){}

void iBattleAct_BeginMoving::BeginAct()
{ 	
	curMovSnd = gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_pActor->Type()].snds.walk, true);		
}

void iBattleAct_BeginMoving::EndAct()
{ 
}

//////////////////////////////////////////////////////////////////////////

iBattleAct_EndMoving::iBattleAct_EndMoving(iBattleGroup* pActor) 
: iBattleAct(fix32::zero), m_pActor(pActor){}

iBattleAct_EndMoving::~iBattleAct_EndMoving() {
	if(curMovSnd != -1) {
		gApp.SndPlayer().Stop(curMovSnd);
		curMovSnd = -1;
	}
}

void iBattleAct_EndMoving::BeginAct()
{ 	
}

void iBattleAct_EndMoving::EndAct()
{ 
	if(curMovSnd != -1) {
		gGame.BattleWrapper()->StopSound(curMovSnd);
		curMovSnd = -1;
	}
}
...
#ifndef SLO_MO 
: iBattleAct(fix32(0.1f)), m_pActor(pActor) {}
#else
: iBattleAct(fix32(0.5f)), m_pActor(pActor) {}
#endif
...
: iBattleAct(fix32(0.4f)), m_pActor(pActor), m_cell(cell), m_penalty(penalty), m_joustBonus(joustBonus), m_bRetail(bRetail), m_luck(luck)
{
	m_targetList.Add(iPair<iBattleGroup*, bool>(pTarget, false));
}

bool iBattleAct_Attack::Normalize()
{
	for (uint32 xx=0; xx<m_targetList.GetSize();) {
		if (!m_targetList[xx].first->IsAlive()) m_targetList.RemoveAt(xx);
		else ++xx;
	}
...
{ 
	...
	gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_pActor->Type()].snds.attack);	
	for (uint32 xx=0; xx<m_targetList.GetSize(); ++xx) {
		m_pActor->Attack(m_targetList[xx].first, false, m_penalty, m_joustBonus, m_luck);		
		if (m_targetList[xx].first->IsAlive())
			gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_targetList[xx].first->Type()].snds.gethit);
		else {
			gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_targetList[xx].first->Type()].snds.die);
			m_targetList[xx].second = true;
		}
		m_targetList[xx].first->SetState(iBattleGroup::RecDamage); 
	}
}
...
{ 
	for (uint32 xx=0; xx<m_targetList.GetSize(); ++xx) {		
		if (!m_targetList[xx].second)
			m_targetList[xx].first->SetState(iBattleGroup::Idle);
		else
			m_targetList[xx].first->SetState(iBattleGroup::Dead);
	}
	m_pActor->SetState(iBattleGroup::Idle);
...
{ 	
	...
	for (uint32 xx=0; xx<m_targetList.GetSize(); ++xx) {		
		m_pActor->Attack(m_targetList[xx].first, aflag, m_penalty, 0, m_luck);
		if (m_targetList[xx].first->IsAlive()) {		
			gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_targetList[xx].first->Type()].snds.gethit);
		} else {
			gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_targetList[xx].first->Type()].snds.die);
...
//////////////////////////////////////////////////////////////////////////
iBattleAct_ShootingStand::iBattleAct_ShootingStand(iBattleGroup* pActor, iBattleGroup* pTarget, bool start) 
: iBattleAct(fix32(0.01f)), m_pActor(pActor), m_start(start)
{	
	m_targetList.Add(pTarget);
}

bool iBattleAct_ShootingStand::Normalize()
{
	for (uint32 xx=0; xx<m_targetList.GetSize();) {
		if (!m_targetList[xx]->IsAlive()) m_targetList.RemoveAt(xx);
		else ++xx;
	}
	return !m_start || m_targetList.GetSize() > 0;
}



void iBattleAct_ShootingStand::BeginAct()
{ 
	if(m_start) {
...
void iBattleAct_ShootingStand::EndAct()
{ 
}

//////////////////////////////////////////////////////////////////////////
iBattleAct_Ammo::iBattleAct_Ammo(iAmmo& ammo, iPoint pos, sint16 sid, fix32 mult, iBattleGroup* pTarget) 
: iBattleAct(fix32(0.0025f) * mult), m_pos(pos), m_ammo(ammo), m_sid(sid)
{
	if(pTarget) {
		m_targetList.Add(pTarget);
		m_bHasTarget = true;
...
void iBattleAct_Ammo::BeginAct()
{ 
	m_ammo.sid = m_sid;
	m_ammo.m_pos = m_pos;
}

bool iBattleAct_Ammo::Normalize()
{
	for (uint32 xx=0; xx<m_targetList.GetSize();) {
		if (!m_targetList[xx]->IsAlive()) m_targetList.RemoveAt(xx);
		else ++xx;
	}
	return !m_bHasTarget || m_targetList.GetSize() > 0;
}

void iBattleAct_Ammo::EndAct()
{
	m_ammo.m_pos = cInvalidPoint;
}
//////////////////////////////////////////////////////////////////////////
...
{ 
	if (m_pSpell->SpellClass() == SPC_DAMAGE) {
		for (uint32 xx=0; xx<m_targetList.GetSize(); ++xx) {
			gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_targetList[xx]->Type()].snds.gethit);	
			m_targetList[xx]->SetState(iBattleGroup::RecDamage); 
		}
	} else if (m_pSpell->SpellClass() == SPC_EARTHQUAKE) {
		for (uint32 xx=0; xx<m_affectedConstrList.GetSize(); ++xx) m_affectedConstrList[xx]->bRecDmg = true;
	} 
...
{ 
	if (m_pSpell->SpellClass() == SPC_EARTHQUAKE) {
		check(m_affectedConstrList.GetSize() && !m_targetList.GetSize());
		for (uint32 xx=0; xx<m_affectedConstrList.GetSize(); ++xx) {
			m_pSpell->Cast(m_pCaster, NULL);
			sint32 damage = 50;
			damage += gGame.Map().Rand(100);
			m_affectedConstrList[xx]->Attack(damage);
			m_affectedConstrList[xx]->bRecDmg = false;
		}
	} else {
		for (uint32 xx=0; xx<m_targetList.GetSize(); ++xx) {
...
//////////////////////////////////////////////////////////////////////////
iBattleAct_FortDmg::iBattleAct_FortDmg(iBattleUnit_Catapult* pActor, iCastleFort* pFort, iCastleFort::ElementType target) 
: iBattleAct(fix32(1.0f)), m_pActor(pActor), m_pFort(pFort), m_target(target) 
{
}

void iBattleAct_FortDmg::BeginAct()
{
	...
	if (m_target != iCastleFort::InvalidElement) {
		...
		gGame.BattleWrapper()->PlaySound(CSND_GETHIT5);
	}
}

void iBattleAct_FortDmg::EndAct()
{ 
	if (m_target != iCastleFort::InvalidElement) {
		iCastleFort::iElement* pTarget = m_pFort->GetElement(m_target);
		check(pTarget);
...
//////////////////////////////////////////////////////////////////////////
iBattleAct_Catapult::iBattleAct_Catapult(iBattleUnit_Catapult* pActor, bool start) 
: iBattleAct(fix32(0.01f)), m_pActor(pActor), m_start(start) 
{
}
...
{
	if(m_start)
		gGame.BattleWrapper()->PlaySound(CSND_THROW_ATTACK);
}

void iBattleAct_Catapult::EndAct()
{
	if(m_start)
		m_pActor->State() = iBattleUnit_Catapult::Shooting;
	else
		m_pActor->State() = iBattleUnit_Catapult::Idle;
}
...
void iBattleAct_Turret::EndAct()
{
	...
	if (m_pTarget->IsAlive()) {
		m_pTarget->SetState(iBattleGroup::Idle);
		gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_pTarget->Type()].snds.gethit);
	} else
		gGame.BattleWrapper()->PlaySound(CREAT_DESC[m_pTarget->Type()].snds.die);
}
//////////////////////////////////////////////////////////////////////////
...
void iBattleArmy::Init(const iArmy& army, iBattleMember* pOwner, sint32 moraleModifier)
{
	m_pOwner = pOwner;
	sint32 mModifier = army.MoraleModifier() + moraleModifier;
	for (uint32 xx=0; xx<7; ++xx) {
		if (army[xx].Type() != CREAT_UNKNOWN) m_groups.Add(new iBattleGroup(army[xx], xx, pOwner, m_pWrapper, mModifier));
	}
}
...
void iBattleArmy::Synchronize(iArmyOwner *pOwner)
{
	for (uint32 xx=0; xx<m_groups.GetSize(); ++xx) m_groups[xx]->Synchronize(pOwner);
}
...
void iBattleArmy::AddGroup( iBattleGroup *pGroup )
{
	check(m_groups.GetSize() < 7);
	m_groups.Add(pGroup);
}
...
iTurnSeq::~iTurnSeq()
{
	if (m_pCurUnit && m_pCurUnit != m_catapult) {
		delete m_pCurUnit;
		m_pCurUnit = NULL;
	}
...
void iTurnSeq::Cleanup()
{
	for (uint32 xx=0; xx<m_turnSeq.GetSize(); ++xx) {
		...
		if(DynamicCast<iBattleUnit_Catapult*>(pUnit) == NULL)
			delete m_turnSeq[xx];
	}
	m_turnSeq.RemoveAll();
}
...
bool iTurnSeq::StepSequence()
{
	...
	if (m_pCurUnit) {
		if(DynamicCast<iBattleUnit_Catapult*>(m_pCurUnit) == NULL)
			delete m_pCurUnit;	
		m_pCurUnit = NULL;
	}
	...
	} else if (DynamicCast<iBattleUnit_Catapult*>(m_pCurUnit)) {
...
void iTurnSeq::NewRoundSequence(iBattleArmy& aa, iBattleArmy& da, iCastleFort* pFort)
{
	...
	if (pFort) {
		if(!m_catapult)
			m_catapult = new iBattleUnit_Catapult(aa.Owner());
...
void iTurnSeq::UpdateSequence(uint32 flags)
{
	...
	sint32 pos[2] = { static_cast<sint32>(m_active[0].Size()-1), static_cast<sint32>(m_active[1].Size()-1) };
	...
	if (m_bHasCatapult) {
		for (uint32 xx=0; xx<m_turnSeq.GetSize(); ++xx) {
			if (m_turnSeq[xx]->Owner()->GetSide() == iBattleMember::Assaulter) {
				m_turnSeq.InsertBefore(xx, m_catapult);
				break;
			}
		}
	}
...
m_turnSeq(pWrapper), 
m_bInited(false), m_iRound(0), m_obsMap(13,11), m_pCastleFort(NULL), m_pCurAct(NULL), m_bAutoBattleForA(false), m_bAutoBattleForD(false)
{
...
bool iBattleEngine::BeginBattle(const iBattleInfo& bi)
{
	m_bi = bi;
	...
	// Init armies	
	m_aArmy.Init(aArmy, m_bi.m_pAssaulter, -m_bi.m_pDefender->FurtSkills().Value(FSK_FEAR) - m_bi.m_pDefender->Army().FearFactor());
	m_dArmy.Init(dArmy, m_bi.m_pDefender, -m_bi.m_pAssaulter->FurtSkills().Value(FSK_FEAR) - m_bi.m_pAssaulter->Army().FearFactor());
	...
	iHero* pAst = m_bi.m_pAssaulter->SpellCaster();
	iHero* pDef = m_bi.m_pDefender->SpellCaster();		
	...
	for (uint32 xx=0; xx<m_aArmy.Count(); ++xx) {
		...
		m_aArmy[xx]->Place(p);
		if (pDef && pDef->Artifacts().HasArtType(ARTT_ICEAMULET))
			m_aArmy[xx]->FurtSkills().Value(FSK_SPEED) -= 2;		
		iCreatBonusList cbl;
		if(pAst && pAst->Artifacts().HasCreatBonus(m_aArmy[xx]->Type(), cbl))
			m_aArmy[xx]->ApplyBonusList(cbl);					
	}
	for (uint32 xx=0; xx<m_dArmy.Count(); ++xx) {
		...
		m_dArmy[xx]->Place(p);
		if (pAst && pAst->Artifacts().HasArtType(ARTT_ICEAMULET))
			m_dArmy[xx]->FurtSkills().Value(FSK_SPEED) -= 2;

		iCreatBonusList cbl;
		if(pDef && pDef->Artifacts().HasCreatBonus(m_dArmy[xx]->Type(), cbl))
			m_dArmy[xx]->ApplyBonusList(cbl);									
	}	

	// lodge gives random mercenaries
	if(iBattleMember_Castle* pBmCastle = dynamic_cast<iBattleMember_Castle*>(bi.m_pDefender)) {
		iCastle *pCastle = pBmCastle->GetCastle();
		iCtlCnst * pLodge = pCastle->FindCnst(CTLCNST_LODGE);
		if(pLodge && pLodge->IsBuilt()) {
			iCreatGroup cg(CREAT_MERCENARY, RandCreaturesCount(CREAT_MERCENARY));
			iBattleMap pmap;
			MakePassMap(pmap);				
			m_dArmy.Summon(cg, FindSummonCell(pmap, iBattleMember::Defender, false));				
		}
	}
	...
	if (CheckBattleState() != BR_NA) {
		iBattleResult br;
		br.m_result = CheckBattleState();
		br.m_defCause = DC_DEFEAT;
		PrepareBattleResult(br);
		EndBattle(br);
		return false;
	}
...
void iBattleEngine::EndBattle(iBattleResult br)
{
	...
	m_aArmy.Synchronize(m_bi.m_pAssaulter->ArmyOwner());
	if(!DynamicCast<iBattleMember_MapEvent*>(m_bi.m_pDefender))
		m_dArmy.Synchronize(m_bi.m_pDefender->ArmyOwner());
...
void iBattleEngine::PrepareDistMap()
{
	if (iBattleUnit_CreatGroup* pCurCreatGroup = DynamicCast<iBattleUnit_CreatGroup*>(m_turnSeq.CurUnit())) {
		...
		fclose(pfile);
		*/
	} else if (DynamicCast<iBattleUnit_Catapult*>(m_turnSeq.CurUnit())){
	} else if (DynamicCast<iBattleUnit_Turret*>(m_turnSeq.CurUnit())){
	}
}
...
sint32 iBattleEngine::CalcShotPenalty(iBattleGroup* pRanger, const iPoint& ptTarget)
{
	...
	if (iHero* pOwner = pRanger->Owner()->SpellCaster()) {
		if (pOwner->HasSpecFlag(SHF_NORANGEPENALTY) || pOwner->Artifacts().HasArtType(ARTT_FASTCROSSBOW)) return 1;
	}
...
void iBattleEngine::PrepareBattleResult(iBattleResult& br)
{
	...
	if (br.m_result == BR_ASSAULTER_WIN) {
		...
		br.m_losHpCas = m_dArmy.HpCasualties();
...
#define UNDEAD(unit) ((unit->Perks() & CPERK_UNDEAD) || (unit->Perks() & CPERK_LIFELESS))

iSimpleArray<iBattleGroup*> iBattleEngine::FindAllGroups(const iPoint& pos, iBattleMember *owner, bool bIncDead, bool bUndead) 
{
	...
	// if we found somebody alive, he'll be our target, not dead ones
	if(res.GetSize() > 0)
		return res;

//	if (bIncDead) {
		// For resurrect spells (try to find dead units only (Hedin))		
		for (xx=0; xx<m_aArmy.Count(); ++xx) {
			if (!m_aArmy[xx]->IsAlive() && 
				m_aArmy[xx]->CanResurrect() && 
				m_aArmy[xx]->IsGroupCell(pos) && (!bUndead && !UNDEAD(m_aArmy[xx]) || 
												   bUndead && UNDEAD(m_aArmy[xx]))) {
		...
		for (xx=0; xx<m_dArmy.Count(); ++xx) {
			if (!m_dArmy[xx]->IsAlive() &&
				m_dArmy[xx]->CanResurrect() && 
				m_dArmy[xx]->IsGroupCell(pos) && (!bUndead && !UNDEAD(m_dArmy[xx]) || 
												   bUndead && UNDEAD(m_dArmy[xx]))) {
...
{
#ifdef MULTIPLAYER
	gMPMgr.OnCombatCastSpell(pSpell, pos, pSpellTarget);	
#endif	
	...
	if (pSpell->NeedTarget(msl)) {
		...
		if (m_pCastleFort) {
			...
			if (m_pCastleFort->GetElement(iCastleFort::MainTower)->state != iCastleFort::Destroyed) walls.Add(m_pCastleFort->GetElement(iCastleFort::MainTower));
			if (m_pCastleFort->GetElement(iCastleFort::Bridge)->state != iCastleFort::Destroyed) walls.Add(m_pCastleFort->GetElement(iCastleFort::Bridge));
			...
			for (uint32 xx=0; (sint32)xx<wmax && walls.GetSize(); ++xx) {
				...
				pAction->m_affectedConstrList.Add(walls[wallIdx]);
				walls.RemoveAt(wallIdx);
			}
		}
		if (pAction->m_affectedConstrList.GetSize()) {
...
void iBattleEngine::Move(const iPoint& pos, uint8 orient)
{
	...
	if (pos == ipos) {
		...
		pf.MakePath(pos, orient, true, m_ActList, CREAT_DESC[pCurGroup->Type()].movAnim);
		...
		iPoint off(0, 0);
		/*if (pCurGroup->Size() == 2 && ) {
				off.x -= TAIL_OFFSET[iorient];
			}
			*/
		iBattleGroup::ORIENT flyOrient = (iBattleGroup::ORIENT)BattleCellsOrient(pCurGroup->Pos().x, pCurGroup->Pos().y, pos.x, pos.y, pCurGroup->Orient());
		if(pCurGroup->Size() == 2) {
			if(flyOrient != pCurGroup->Orient() && pCurGroup->CanMove(pos.x - TAIL_OFFSET[flyOrient], pos.y))
				off.x -= TAIL_OFFSET[flyOrient];			
			if(!pCurGroup->CanMove(pos.x + off.x + TAIL_OFFSET[flyOrient], pos.y)) {
				off.x -= TAIL_OFFSET[flyOrient];
				
			}		
		}

		pf.MakeDirectPath(pCurGroup->Pos(), pos + off, iorient, true, m_ActList, CREAT_DESC[pCurGroup->Type()].movAnim);		
...
iPoint iBattleEngine::GetThroughHex(const iPoint& pos, uint16 mdir)
{
	sint32 yseq = (pos.y&1);
...
void iBattleEngine::MakeMeleeSequence(iBattleGroup* pActor, iBattleGroup* pTarget, const iPoint& pos, sint32 penalty, uint16 mdir, bool bRetail, iBattleActList& actList, iPoint meleePoint)
{
	// Check luck status
	...
	// attack target
	iBattleAct_Attack* pAttackAct = new iBattleAct_Attack(pActor, pos, pTarget, penalty, pActor->CalcJoustingBonus(meleePoint), bRetail, luck);
	...
	if (pActor->HasPerk(CPERK_ADJACENTATTACK)) {
		...
		for (uint32 xx=0; xx < 6; ++xx) {
			sint32 cx = spos.x + HEX_MELEE_NEBS1[yseq][xx][0];
			sint32 cy = spos.y + HEX_MELEE_NEBS1[yseq][xx][1];
			if (pActor->PassMap().IsValidPos(cx,cy) && pActor->PassMap().GetAt(cx,cy) == CT_ENEMYC) {
				iBattleGroup* pNebTarget = FindGroup(iPoint(cx,cy));
				check(pNebTarget);
				pAttackAct->m_targetList.AddUnique(iPair<iBattleGroup*, bool>(pNebTarget, false));
			}
		}
	}
	...
	if (pActor->HasPerk(CPERK_TWOHEXATTACK)) {
		if (iBattleGroup* pNebTarget = FindGroup(GetThroughHex(pos, mdir))) {
				if (pNebTarget != pActor) pAttackAct->m_targetList.AddUnique(iPair<iBattleGroup*, bool>(pNebTarget, false));
			}		
	}
}
...
void iBattleEngine::MakeShootSequence(iBattleGroup* pActor, iBattleGroup* pTarget, const iPoint& pos, sint32 penalty, iBattleActList& actList)
{
	...
	// arrow flight
	iPoint src = iBattleView::Map2Screen(pActor->Pos()) + iPoint(0, -15);
	iPoint dst = iBattleView::Map2Screen(pTarget->Pos()) + iPoint(0, -15);

	 src.y += CREAT_DESC[pActor->Type()].shootStartPoint.y;

	 if(pActor->Orient() == iBattleGroup::Left) {
		dst.x += 16;
		src.x -= CREAT_DESC[pActor->Type()].shootStartPoint.x;
		src.x += 5;
	 } else {
		src.x += CREAT_DESC[pActor->Type()].shootStartPoint.x;
		src.x += 0;
...
#define STEP 4
	float dx, dy;
	sint16 steps;
	sint16 xsteps = iABS<sint16>(dst.x - src.x) / STEP; // 1 frame every 4 px	
	sint16 ysteps = iABS<sint16>(dst.y - src.y) / STEP; // 1 frame every 4 px
	if(xsteps > ysteps) {
		steps = xsteps;
		dx = (sint16)(dst.x > src.x ? (-STEP) : STEP);
		dy = (dst.y > src.y ? (-1.0f) : 1.0f) * iABS<sint16>(dst.y - src.y) / steps;				
	} else {
		steps = ysteps;
		dy = (sint16)(dst.y > src.y ? (-STEP) : STEP);
		dx = (dst.x > src.x ? (-1.0f) : 1.0f) * iABS<sint16>(dst.x - src.x) /steps;			
	}

	m_ActList.AddAction(new iBattleAct_ShootingStand(pActor, pTarget, true));
	//if(m_ammo.sid != -1) {
		if(CREAT_DESC[pActor->Type()].shootAnim == SA_FLY) {
			m_ammo.reversed = (dst.x < src.x);
			for(sint32 x=0; x<steps; x++)
				m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, iPoint(src.x - (sint16)(x * dx), src.y - (sint16)(x * dy)), CREAT_DESC[pActor->Type()].ammoSprite, fix32(1.0f), pTarget));
		}
		else if(CREAT_DESC[pActor->Type()].shootAnim == SA_INPLACE_2) {
			m_ammo.reversed = false;			
			m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, dst + iPoint(18, -15), CREAT_DESC[pActor->Type()].ammoSprite, fix32(20.0f), pTarget));
			m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, dst + iPoint(18, -15), CREAT_DESC[pActor->Type()].ammoSprite + 1, fix32(20.0f), pTarget));
		} else if(CREAT_DESC[pActor->Type()].shootAnim == SA_INPLACE_3) {
			m_ammo.reversed = false;			
			m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, dst + iPoint(18, 9), CREAT_DESC[pActor->Type()].ammoSprite, fix32(10.0f), pTarget));
			m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, dst + iPoint(18, 9), CREAT_DESC[pActor->Type()].ammoSprite + 1, fix32(10.0f), pTarget));
			m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, dst + iPoint(18, 9), CREAT_DESC[pActor->Type()].ammoSprite + 2, fix32(10.0f), pTarget));
		} 
	//}
	...
	m_ActList.AddAction(new iBattleAct_ShootingStand(pActor, pTarget, false));
	...
	if (pActor->HasPerk(CPERK_LICHSHOOT) ) {
		...
		for (uint32 xx=0; xx < 6; ++xx) {
			...
			if (pActor->PassMap().IsValidPos(cx,cy) && (pActor->PassMap().GetAt(cx,cy) == CT_ENEMYC || pActor->PassMap().GetAt(cx,cy) == CT_FRIENDLYC)) {
				iBattleGroup* pNebTarget = FindGroup(iPoint(cx,cy));
				check(pNebTarget);
				if ( !pNebTarget->HasPerk(CPERK_UNDEAD) && !pNebTarget->HasPerk(CPERK_LIFELESS) ) pShootAct->m_targetList.AddUnique(iPair<iBattleGroup*, bool>(pNebTarget, false));
			}
		}
	}
}
...
{
#ifdef MULTIPLAYER
	gMPMgr.OnCombatAttack(pos, mdir);
#endif
    ...
    iPoint newPos, savedPos;    
    savedPos = pCurGroup->Pos();
    newPos = mp;
	...
	// Check condition if we're not need to move
		if (pCurGroup->Size() == 1 && pCurGroup->Pos() == mp) {
			...
			iBattlePF pf(pCurGroup);
			pf.MakePath(mp, aor, false, m_ActList, CREAT_DESC[pCurGroup->Type()].movAnim);
		}
	} else if (pCurGroup->TransType() == TRANS_FLY) {
		iBattleGroup::ORIENT flyOrient = (iBattleGroup::ORIENT)BattleCellsOrient(pCurGroup->Pos().x, pCurGroup->Pos().y, mp.x, mp.y, pCurGroup->Orient());
			/*if (pCurGroup->Size() == 2 &&				
				mp.y == pTarget->Pos().y && (mp.x >= pTarget->Pos().x && flyOrient == iBattleGroup::Right ||
				 mp.x < pTarget->Pos().x && flyOrient == iBattleGroup::Left)) {
				mp.x += TAIL_OFFSET[aor];
			}*/

		if(pCurGroup->Size() == 2 && aor != flyOrient)
			mp.x += TAIL_OFFSET[aor];
		...
		if (pCurGroup->IsGroupCell(mp)) {
			...
			// Move to destination point
			pf.MakeDirectPath(pCurGroup->Pos(),  mp, aor, true, m_ActList, CREAT_DESC[pCurGroup->Type()].movAnim);		
		}
	}
	
	// target rotates to assaulter (if required)
	if (pTarget->Orient() == aor) 
		m_ActList.AddAction(new iBattleAct_Rotate(pTarget));

	// Attack
	MakeMeleeSequence(pCurGroup, pTarget, pos, 1, mdir, false, m_ActList, mp);
	...
	if (pTarget->CanRetail() && !pCurGroup->HasPerk(CPERK_NONRETALATTACK)) {
		...
		MakeMeleeSequence(pTarget, pCurGroup, rpos, penalty, rdir, true, m_ActList, pTarget->Pos());
...
//////////////////////////////////////////////////////////////////////////
// 06.08.2010 CPERK_RETURN
// the creature returns to its initial position
// only fly mode is supported now
//////////////////////////////////////////////////////////////////////////
    if (pCurGroup->HasPerk(CPERK_RETURN)) 
    {
        iBattleActList retList;

        mp = savedPos;
        if (pCurGroup->TransType() == TRANS_FLY)
        {
            iBattleGroup::ORIENT flyOrient = (iBattleGroup::ORIENT)BattleCellsOrient(newPos.x, newPos.y, mp.x, mp.y, pCurGroup->Orient());
            ...
            {
                if (pCurGroup->Orient() != aor) 
                    retList.AddAction(new iBattleAct_Rotate(pCurGroup));
            } 
            else
            {
                iBattlePF pf(pCurGroup);
                // Move to destination point
                if (flyOrient == iBattleGroup::Left)
                    flyOrient = iBattleGroup::Right;
                else
                    flyOrient = iBattleGroup::Left;
                pf.MakeDirectPath(newPos, mp, flyOrient, true, retList, CREAT_DESC[pCurGroup->Type()].movAnim);		                
            }

            while (retList.Count())
            {
                iBattleAct *ba = retList.StepAction();
                m_ActList.AddAction(ba);
            }
        }
    }
//////////////////////////////////////////////////////////////////////////
...
{
#ifdef MULTIPLAYER 
	gMPMgr.OnCombatCatapultShot(target);
#endif
	...
	for (sint32 xx=0; xx<CATAPULT_PROPS[bal][2]; ++xx) {
		m_ActList.AddAction(new iBattleAct_Delay());
		m_ActList.AddAction(new iBattleAct_Catapult(pCatapult, true));
		iPoint catPos = iBattleView::Map2Screen(iPoint(1, 9));
		iPoint elemPos = iBattleView::FortElemHitPos(iBattleView::Map2Screen(iPoint(iCastleFort::ElCells[target][0], iCastleFort::ElCells[target][1])), target, iCastleFort::Normal);
		
		double a, b, c;
		double x1 = catPos.x, y1 = catPos.y, x2 = elemPos.x, y2 = elemPos.y, x3 = catPos.x + (elemPos.x - 
			catPos.x) / 2, y3 = 10;
		a = (y3 - (x3*(y2 - y1) + x2*y1 - x1*y2) / (x2 - x1)) / (x3 * (x3 - x1 - x2) + x1*x2);
		b = (y2 - y1) / (x2 - x1) - a*(x1 + x2);
		c = (x2*y1 - x1*y2) / (x2 - x1) + a*x1*x2;
		m_ammo.reversed = false;
		for(sint32 x=catPos.x + 3; x<elemPos.x; x++)
			m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, iPoint(x, (sint32)(a*x*x + b*x + c)), PDGG(SHOOT_AMMO) + 2, fix32(1.0f), NULL));
		
		m_ActList.AddAction(new iBattleAct_FortDmg(pCatapult, m_pCastleFort, target));
		m_ActList.AddAction(new iBattleAct_Catapult(pCatapult, false));
		m_ActList.AddAction(new iBattleAct_Delay());
		target = CastleFort()->GetTarget();
	}
...
{
#ifdef MULTIPLAYER
	gMPMgr.OnCombatTurretShot(pTarget->Pos());
#endif
	check(m_pCastleFort);
	...
	// arrow flight
	iPoint src = iBattleView::Map2Screen(iPoint(iCastleFort::ElCells[pCurTurret->m_et][0], iCastleFort::ElCells[pCurTurret->m_et][1]));
	iPoint dst = iBattleView::Map2Screen(pTarget->Pos()) + iPoint(0, -15);
	...
	float dx, dy;
	sint16 steps;
	sint16 xsteps = iABS<sint16>(dst.x - src.x) / 4; // 1 frame every 4 px	
	sint16 ysteps = iABS<sint16>(dst.y - src.y) / 4; // 1 frame every 4 px	;
	if(xsteps > ysteps) {
		steps = xsteps;
		dx = (sint16)(dst.x > src.x ? (-4) : 4);
		dy = (dst.y > src.y ? (-1.0f) : 1.0f) * iABS<sint16>(dst.y - src.y) / steps;				
	} else {
		steps = ysteps;
		dy = (sint16)(dst.y > src.y ? (-4) : 4);
		dx = (dst.x > src.x ? (-1.0f) : 1.0f) * iABS<sint16>(dst.x - src.x) /steps;			
	}

	m_ammo.reversed = true;
	for(sint32 x=0; x<steps; x++)
		m_ActList.AddAction(new iBattleAct_Ammo(m_ammo, iPoint(src.x - (sint16)(x * dx), src.y - (sint16)(x * dy)), PDGG(SHOOT_AMMO) + 3, fix32(1.0f), pTarget));
...
const bool iBattleEngine::OnShip() const 
{   
    if (iBattleMember_Hero* pBMHero = DynamicCast<iBattleMember_Hero*>(m_bi.m_pDefender))
    {
        if (pBMHero->GetHero()->InShip())
            return true;
    }       

    if (iBattleMember_VisCnst* pBMVisCnst = DynamicCast<iBattleMember_VisCnst*>(m_bi.m_pDefender))
    {
        if (pBMVisCnst->GetVisCnst()->Proto()->NameKey() == TRID_OBJ_SHIPWRECK)
            return true;
    }
    return false;                
}
...
// Cotulla: added separate autocombat for each players in dual-mode
void iBattleEngine::SetAutobattle(bool bAutoBattle) 
{ 
	if(bAutoBattle) {
		if (TurnSeq().CurUnit()->Owner()->Owner() == m_aArmy.Owner()->Owner())
			m_bAutoBattleForA = bAutoBattle; 
		else
			m_bAutoBattleForD = bAutoBattle; 
	} else {
		m_bAutoBattleForD = m_bAutoBattleForA = false;
	}
}

bool iBattleEngine::IsAnyAutoBattle()  
{
	return m_bAutoBattleForA || m_bAutoBattleForD;
}

bool iBattleEngine::IsAutoBattle()  
{ 
    if (TurnSeq().CurUnit()->Owner()->Owner() == m_aArmy.Owner()->Owner())
        return m_bAutoBattleForA; 
    else
        return m_bAutoBattleForD;
}
