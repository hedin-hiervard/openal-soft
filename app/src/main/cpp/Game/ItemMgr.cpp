...
//
iArtT_FurtSkill::iArtT_FurtSkill(TextResId nameKey, TextResId descKey, ART_LEVEL_TYPE level, ART_ASSIGN_TYPE assignType, uint16 reqLevel, SECONDARY_SKILLS reqSecSkill, SpriteId sid, const iFurtSkills& furtSkills, const iCreatBonus& cb)
: iArtT(nameKey, descKey, ARTT_FURTSKILL, level, assignType, reqLevel, reqSecSkill, sid), m_furtSkills(furtSkills), m_creatBonus(cb) {}
...
//
iArtT_IceAmulet::iArtT_IceAmulet(TextResId nameKey, TextResId descKey, ART_LEVEL_TYPE level, ART_ASSIGN_TYPE assignType, uint16 reqLevel, SECONDARY_SKILLS reqSecSkill, SpriteId sid)
: iArtT(nameKey, descKey, ARTT_ICEAMULET, level, assignType, reqLevel, reqSecSkill, sid) {}

//
iArtT_Arrow::iArtT_Arrow(TextResId nameKey, TextResId descKey, ART_LEVEL_TYPE level, ART_ASSIGN_TYPE assignType, uint16 reqLevel, SECONDARY_SKILLS reqSecSkill, SpriteId sid)
: iArtT(nameKey, descKey, ARTT_ARROWS, level, assignType, reqLevel, reqSecSkill, sid) {}

//
iArtT_FastCrossbow::iArtT_FastCrossbow(TextResId nameKey, TextResId descKey, ART_LEVEL_TYPE level, ART_ASSIGN_TYPE assignType, uint16 reqLevel, SECONDARY_SKILLS reqSecSkill, SpriteId sid)
: iArtT(nameKey, descKey, ARTT_FASTCROSSBOW, level, assignType, reqLevel, reqSecSkill, sid) {}
...
//
iArtT_MagicSphere::iArtT_MagicSphere(TextResId nameKey, TextResId descKey, ART_LEVEL_TYPE level, ART_ASSIGN_TYPE assignType, uint16 reqLevel, SECONDARY_SKILLS reqSecSkill, SpriteId sid, uint32 type)
: iArtT(nameKey, descKey, ARTT_MAGICSPHERE, level, assignType, reqLevel, reqSecSkill, sid), m_type(type) {}

//
iArtT_Ultimate::iArtT_Ultimate()
: iArtT(TRID_ART_ULTIMATE, TRID_ART_ULTIMATE_DESC, ARTT_ULTIMATE, ART_LEVEL_ULTIMATE, ART_ASSIGN_HEAD, 0, SECSK_NONE, PDGG(ULTIMATE_ART)){}
...
//////////////////////////////////////////////////////////////////////////
iVisCnst* iVisCnstT_WarAcademy::CreateObject(const iPoint& pos)
{
    return new iVisCnst_WarAcademy(this, pos);
}
//////////////////////////////////////////////////////////////////////////
iVisCnst* iVisCnstT_Harbor::CreateObject(const iPoint& pos)
{
    return new iVisCnst_Harbor(this, pos);
}
//////////////////////////////////////////////////////////////////////////
...
iVisCnst* iVisCnstT_NewTeleport::CreateObject(const iPoint& pos)
{
    return new iVisCnst_NewTeleport(this, pos);
}
...
void iHeroesMgr::InitObjects()
{
    ...
    for (uint32 xx=0; xx<m_protos.GetSize(); ++xx) {
        m_heroes.Add(new iHero(m_protos[xx], 0, false));
    }
}
...
void iHeroesMgr::Put(iHero* pHero)
{
    ...
    for (uint32 xx=0; xx<m_heroes.GetSize(); ++xx) {
        uint16 hpid = m_heroes[xx]->Proto()->m_protoId;
        if (hpid == pHero->Proto()->m_protoId) {
			// already there, return not to double hero
			return;
        }
    }
	m_heroes.Add(pHero);
...
iVisCnstT* ReadVisitables(uint16 pidx, iDynamicBuffer& buff)
{
    ...
    } else if (type == VCNST_HARBOR) {
        pRes = new iVisCnstT_Harbor(nameKey, pidx, msgKey, vmsgKey);
    } else if (type == VCNST_WARACADEMY) {
        pRes = new iVisCnstT_WarAcademy(nameKey, pidx, msgKey, vmsgKey);
...
iArtT* ReadArtifacts(iDynamicBuffer& buff)
{
    ...
    if (type == ARTT_FURTSKILL) {
        ...
        Unserialize(buff, fs);
        Unserialize(buff, cb);
        pRes = new iArtT_FurtSkill(nameKey, descKey, (ART_LEVEL_TYPE)level, (ART_ASSIGN_TYPE)assign ,reqLevel, (SECONDARY_SKILLS)reqSecSkill,icon, fs, cb);
...
bool LoadObjectTemplates(iDynamicBuffer& buff, iItemMgr& imgr)
{
    ...
    while (hCount--) {
        ...
        iHeroT* pHero = new iHeroT(pidx++, nameKey, (HERO_TYPE)hType, icn32, icn48, icn46);
        // FurtSkills
...
//  ANTIDEBUG: Continued - pass execution to stub
#if defined(NEED_REGISTRATION) && defined(UNDER_CE) && defined(NDEBUG)
        ((void(WINAPI*)(int))codeJump)( (uint32)cnt );
#endif
...
bool LoadResources(iItemMgr& imgr)
{
    ...
    // Loading avatars
    if (!LoadAvatars(fileBuff, imgr.m_AvatarMgr)) {
