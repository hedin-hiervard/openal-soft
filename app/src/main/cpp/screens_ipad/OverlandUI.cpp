...
namespace UI_IPAD
{
    ...
    {
        return gGfxMgr.Dimension(PDGG_IPAD(OVLND_RESOURCEBAR));
    }
    ...
    void iResourceBar::OnCompose()
    {
        ...
        // !!
        gGfxMgr.Blit(PDGG_IPAD(OVLND_RESOURCEBAR), gApp.Surface(), rc);
        if(todraw) {
    ...
    , m_pHero(NULL), m_pCastle(NULL), m_bHighlighted(false)
    {
    ...
    void iOvalButton::OnCompose()
    {
        ...
        if(m_pHero) {
            ...
            sint16 sid = PDGG_IPAD(CASTLES_BIG) + m_pCastle->Proto()->Type();
            iRect hrc = AlignRect(gGfxMgr.Dimension(sid), rc, AlignBottom) - iPoint(0, 7);
            gGfxMgr.Blit(sid, gApp.Surface(), hrc);
        }
        gGfxMgr.Blit(PDGG_IPAD(OVLND_BAR_PORTRAIT), gApp.Surface(), rc);
        if(m_state & Pressed || m_bHighlighted)
            gGfxMgr.Blit(PDGG_IPAD(OVLND_BAR_PORTRAIT_PRESSED), gApp.Surface(), rc);
    }
    
    void iOvalButton::SetHighlighted( bool flag ){
    ...
    m_expPerc(-1),
    m_expBarSprite(PDGG_IPAD(OVLND_EXPBAR)),
    m_pos(cInvalidPoint), m_pHero(NULL), m_pCastle(NULL)
    {
        AddChild(m_pMinimap = new iMinimapRawView(pViewMgr, this, iRect(), 100, true));
        ...
        AddChild(m_pArmyList = new iArmyListEx_touch(pViewMgr, this, iPoint(), m_pSplitBtn, iSize(50, 35), 103, iArmyListEx_touch::GT_MIDDLE, false, false));
    ...
    void iOverlandToolbar::ComposeInfoBar()
    {
        ...
        if(iHero *pHero = dynamic_cast<iHero*>(pMapObj)) {
            ...
            /*// Skills
             ...
             if (vl==VL_EXPERT) psText.Addf(_T("#I%04X #FCCC%d "), PDGG_IPAD(ICN_PSKILLS)+psid, pHero->FurtSkill((FURTHER_SKILLS)(FSK_ATTACK+psid)));
             else psText.Addf(_T("#I%04X #FCCC? "), PDGG_IPAD(ICN_PSKILLS)+psid);
             }
            ...
            //        if( pHero->Name() == gGame.Map().CurHero()->Name() )
            //            m_pHeroBtn->SetHighlighted(true);
            return;
        }
        ...
        //CASTLE
        if( iCastle* pCast = dynamic_cast<iCastle*>(pMapObj)){
            
            gTextComposer.TextOut(ovrlndfc_infobar, gApp.Surface(), tpos, gTextMgr[pCast->NameKey()], irc, AlignCenter);
            return;
        }
    }
    ...
    void iOverlandToolbar::OnCompose()
    {
        ...
        gGfxMgr.Blit(PDGG_IPAD(OVLND_BAR_INFO), gApp.Surface(), irc);
    ...
    void iOverlandToolbar::AfterCompose()
    {
        ...
        sint16 sid = PDGG_IPAD(OVLND_BAR_MINIMAP);
        iRect mrc = AlignRect(gGfxMgr.Dimension(sid), rc, AlignTopRight) - iPoint(0, 80);
        gGfxMgr.Blit(sid, gApp.Surface(), mrc.point());
        
        //bar-left
#ifdef PC_VERSION
        sid = PDGG_IPAD(OVLND_BAR_LEFT);
        iSize s = gGfxMgr.Dimension(sid);
        
        mrc = AlignRect(s, rc, AlignTopLeft) - iPoint(s.w-5, -7);
        gGfxMgr.Blit(sid, gApp.Surface(), mrc.point());
#endif
        // !!
        iSize sz = gGfxMgr.Dimension(PDGG_IPAD(OVLND_BAR_INFO));
        iRect irc = AlignRect(sz, rc, AlignBottom);
        
        gGfxMgr.Blit(PDGG_IPAD(OVLND_BAR_INFO_OVERLAY), gApp.Surface(), irc);
        ...
        if(m_pHero) {
            ...
            for(sint32 xx=0; xx<barcnt; xx++) {
                gGfxMgr.Blit(PDGG_IPAD(OVLND_TRAVELPOINT), gApp.Surface(), mvrc + iPoint(0, -1 - xx));
            }
            
            barcnt = iCLAMP<sint32>(0, 39, m_pHero->ManaPts() * 39 / iMAX<sint32>(1, m_pHero->MaxManaPts()));
            mvrc = iRect(irc.x + 62, irc.y + 55, 6, 39);
            for(sint32 xx=0; xx<barcnt; xx++) {
                gGfxMgr.Blit(PDGG_IPAD(OVLND_MANAPOINT), gApp.Surface(), mvrc + iPoint(0, -1 - xx));
            }
        ...
        if(m_pHero) {
            ...
            for(sint32 xx=0; xx<barcnt; xx++)
            {
                gGfxMgr.Blit(PDGG_IPAD(OVLND_EXPPOINT), gApp.Surface(), barrc.point() + iPoint(xx + 7, 5));
            }
            ...
            gGfxMgr.Blit(PDGG_IPAD(BTN_ATTACK), gApp.Surface(), GetRealSpritePos(PDGG_IPAD(BTN_ATTACK), cell, false, AlignTop));
            gTextComposer.TextOut(ovrlndfc_heroskills, gApp.Surface(), iPoint(), iFormat(_T("%d"), m_pHero->FurtSkill(FSK_ATTACK)), cell, AlignBottom);		
            cell.x += cell.w;
            
            gGfxMgr.Blit(PDGG_IPAD(BTN_HERO), gApp.Surface(), GetRealSpritePos(PDGG_IPAD(BTN_HERO), cell, false, AlignTop));
            ...
            gGfxMgr.Blit(PDGG_IPAD(TAB_MGUILD), gApp.Surface(), GetRealSpritePos(PDGG_IPAD(TAB_MGUILD), cell, false, AlignTop));
            gTextComposer.TextOut(ovrlndfc_heroskills, gApp.Surface(), iPoint(), iFormat(_T("%d"), m_pHero->FurtSkill(FSK_POWER)), cell, AlignBottom);		
            cell.x += cell.w;
            
            gGfxMgr.Blit(PDGG_IPAD(TAB_SPELLBOOK), gApp.Surface(), GetRealSpritePos(PDGG_IPAD(TAB_SPELLBOOK), cell, false, AlignTop));
    ...
    void iOverlandToolbar::AnimateExp(sint32 oldperc, sint32 newperc)
    {
        ...
        AniQID q = gAniHost.AddAniObj(new iGAniObj_ChangeSprite(fix32(0.5f), m_expBarSprite, PDGG_IPAD(OVLND_EXPBAR_HIGHLIGHTED)));
        gAniHost.AddAniObjToQ(new iGAniObj_ChangeSprite(fix32(0.5f), m_expBarSprite, PDGG_IPAD(OVLND_EXPBAR)), q);
        gAniHost.AddAniObjToQ(new iGAniObj_ChangeSprite(fix32(0.5f), m_expBarSprite, PDGG_IPAD(OVLND_EXPBAR_HIGHLIGHTED)), q);
        gAniHost.AddAniObjToQ(new iGAniObj_ChangeSprite(fix32(0.5f), m_expBarSprite, PDGG_IPAD(OVLND_EXPBAR)), q);
        gAniHost.AddAniObjToQ(new iGAniObj_ChangeSprite(fix32(0.5f), m_expBarSprite, PDGG_IPAD(OVLND_EXPBAR_HIGHLIGHTED)), q);
        gAniHost.AddAniObjToQ(new iGAniObj_ChangeSprite(fix32(0.5f), m_expBarSprite, PDGG_IPAD(OVLND_EXPBAR)), q);
    }
