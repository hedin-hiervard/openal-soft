...
//		gApp.Surface().FillRect(iRect(rc.x,rc.y,34,22),cColor_Black);
		...
		gGfxMgr.Blit(sid,gApp.Surface(), iPoint(rc.x+1,rc.y + 
												10
												));
		...
		rc.y+=15;
		if (state == Occupied) {			
			gTextComposer.TextOut(listfc_text,gApp.Surface(),rc,pCastle->Visitor()->Name(),rc,AlignTop);
		} else if (state == Normal) {			
			gTextComposer.TextOut(listfc_text, gApp.Surface(),rc,gTextMgr[TRID_UNOCCUPIED],rc,AlignTop);
		} else {
			check(0);
		}
	}
...
void iDlg_TownList::OnCreateDlg()
{
	...
	AddChild(m_pTownList = new iTownListBox(m_pMgr, this, iRect(clRect.x+5, clRect.y+20, clRect.w-5-1, 130), 101, m_pOwner));
