...
void iDlg_BattleHero::OnCreateDlg()
{
	...
	rc.x+=rc.w + 3;
	rc.w = 92;
	AddChild(new iTextButton(m_pMgr,this,rc,TRID_AUTO_COMBAT, AutoBattle));
	rc.x+=rc.w + 3;
	rc.w = 52;
}
...
void iDlg_BattleHero::DoCompose(const iRect& clRect)
{
	...
	iTextComposer::FontConfig fch(FS_LARGE, RGB16(255,192,128));
	iTextComposer::FontConfig fcv(FS_LARGE, RGB16(255,255,0));	
...
void iDlg_BattleHero::OnMouseUp(const iPoint &pos, MouseId mID)
{
	EndDialog(0);
}
