...
bool iSoundPlayer::Init()
{
	...
	MMRESULT res = ::waveOutOpen( &m_hWaveOut, WAVE_MAPPER, &wfex, m_idWorkThread, (DWORD)this, CALLBACK_THREAD );
...
void iSoundPlayer::Close()
{
	if (m_hWaveOut) {
		...
		for(uint32 xx=0; xx<m_sampleLibrary.GetSize(); xx++){
		
//			if( m_sampleLibrary[xx].data )
//				delete m_sampleLibrary[xx].data;
		}
	}
}
...
uint32 iSoundPlayer::LoadSample(sint8* pSample, uint32 smpLen, uint32 flags)
{
	...
	info.data = pSample;
	info.smpLen = smpLen;
	m_sampleLibrary.Add(info);
	return m_sampleLibrary.GetSize() - 1;
}
...
uint32  iSoundPlayer::PlaySample(uint32 sid, uint32 flags)
{
	...
	for (uint32 n = 0; n<STREAM_COUNT; ++n) {
		if (m_streams[n].m_state == iStream::Available) {
			::EnterCriticalSection(&m_csGuard);
			m_streams[n].PushSample(m_sampleLibrary[sid].data, m_sampleLibrary[sid].smpLen, flags);
			::LeaveCriticalSection(&m_csGuard);
			return n;
		}
	}
...
uint32 iSoundPlayer::ReplaceSample(uint32 streamId, uint32 sid)
{
	check(m_hWaveOut);
	check(streamId>=0 && streamId<STREAM_COUNT);
	...
	m_streams[streamId].PushSample(m_sampleLibrary[sid].data, m_sampleLibrary[sid].smpLen, PlayLoop);
	::LeaveCriticalSection(&m_csGuard);
...
void iSoundPlayer::Stop(uint32 streamId)
{
	check(streamId >= 0 && streamId < STREAM_COUNT);
	::EnterCriticalSection(&m_csGuard);
