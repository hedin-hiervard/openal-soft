#ifndef GXLIB_GXL_DIB_H_
#define GXLIB_GXL_DIB_H_
...
public:
	...
	void Init(const iSize& siz, Type dType, void * backbuffer);
	void Cleanup();
	...
	void Circle(const iPoint& center, uint32 radius, pixel clr, bool bFill);
	void Triangle(const iPoint& p1, const iPoint& p2, const iPoint& p3, pixel clr);

	// Copy/Blend
	void CopyToDibXY(iDib& dib, const iPoint& pos, pixel ck) const;
	...
	void Allocate(void * backbuffer, const iSize& siz);
