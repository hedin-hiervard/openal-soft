#ifndef GXLIB_GXL_MEMORY_H_
#define GXLIB_GXL_MEMORY_H_
...
public:
	...
	void Assign(T* backbuffer, uint32 siz)
	{
		//Clean();
		m_Siz = siz;
		m_pBuff = backbuffer;
...
public:
	...
	bool Write( const iStringW& s )	// use NULL terminator
	{
		...
		if ( !s.Empty() ) {
			uint32 len = s.Length();
			res = Write( len );
			sint16 *tempbuf = new sint16[len + 1];
			const iCharW *buf = s.CStr();
			for(uint32 x=0; x < len; x++)
				tempbuf[x] = (sint16)(buf[x]);
			if ( len )	res &= (Write( tempbuf, len*sizeof(sint16)) > 0 );
			delete[] tempbuf;
		} else {
			uint32 len = 0;
			res = Write( len );
