...
bool iDisplay::Init(HWND hwnd, const iSize& siz, uint32 flags)
{
		...
		m_Siz = siz;
		//iPhone_initSurface(siz.w, siz.h);
//		pthread_mutex_lock(&initMutex);
		//void *buf = iPhone_getSurface();
		//return;
...
void iPhone_updateScreenRect(const unsigned short* screen, int x1, int y1, int x2, int y2);
}*/
...
#endif
		buf = iPhone_getSurface();
//		textW = getSizeNextPOT(m_Siz.w);
//		textH = getSizeNextPOT(m_Siz.h);
	} else {
		//pthread_mutex_lock(&surfaceMutex);
	}
	//iPhone_updateScreenRect(pSrc, 0, 0, 320, 480);
