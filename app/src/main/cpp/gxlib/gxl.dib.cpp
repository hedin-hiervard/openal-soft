...
inline void BlitDibBlockCK(iDib::pixel *dst, const iDib::pixel *src, uint32 size, iDib::pixel ck)
{
...
inline void BlitDibBlockConstAlpha(iDib::pixel *dst, const iDib::pixel *src, uint8 a, uint32 size)
{
...
void iDib::Init(const iSize& siz, Type dType, void * backbuffer)
{
	m_dibType = dType;
	Allocate(backbuffer, siz);
}
...
void iDib::Allocate(void * backbuffer, const iSize& siz)
{
	check(m_RGB.IsClean());
	m_Siz = siz;
	m_RGB.Assign((short unsigned int*)backbuffer, siz.w*siz.h);
}
...
void iDib::Circle(const iPoint& center, uint32 radius, pixel clr, bool bFill)
{
	for(sint32 xx=-(sint32)radius; xx<=(sint32)radius; xx++)
	{
		...
		if(bFill)
			for(sint32 yy=-span; yy<=span; yy++) {
				PutPixel(center.x + xx, center.y + yy, clr);
			}
		else {
...
void iDib::CopyToDibXY(iDib& dib, const iPoint& pos, uint8 a) const
{
	...
	for (uint32 yy=0; yy<dst_rect.h; yy++) {
		BlitDibBlockConstAlpha(dst_clr,src_clr,a,dst_rect.w);
		src_clr+=m_Siz.w;
		dst_clr+=dib.GetWidth();
	}
}
