...
: iBaseGameDlg(pViewMgr)
{
	...
	m_devId = (uint32) (xxc::store_find( XXC_GET_SEC(), SEC_HWID_CNST )->data);
	//m_devId = 0xBBDC203A;
#else
	m_devId = 0xDEADBABE; //DEADBABE
#endif
...
void iDlg_Register::OnCreateDlg()
{
	...
	ypos += iBaseGameDlg::GetTopOffset();
	AddChild(new iPHLabel(m_pMgr, iRect(clRect.x, ypos,clRect.w, 15), iFormat(_T("%08X"), m_devId), AlignCenter, iTextComposer::FontConfig(FS_MEDIUM, cColor_White)));
	ypos += 18;
...
//#define HANDANGO_VER
		...
		iIconDlg tdlg(m_pMgr, gTextMgr[TRID_REG_REGISTRATION], gTextMgr[TRID_REG_HELP_POCKETLAND], PDGG(BANNER_POCKETLAND), PID_NEUTRAL);
