#ifndef PH_COMMON_COMMON_H_
#define PH_COMMON_COMMON_H_

/*
 *  Common defines and constants
 */
const uint32 EMAP_FILE_HDR_KEY = 0x76235278;
// 0x23 - new format with ships support
// 0x24 - added victory condition, level cap, hero customization, requirements for monsters and buildings, new time event format
// 0x25 - added requirements and rewards for monsters and buildings, hero 'unexping' feature
const uint32 EMAP_FILE_VERSION = 0x25;

const uint32 GMAP_FILE_HDR_KEY = 'G' | ('M'<<8) | ('A'<<16) | ('P'<<24);
const uint16 GMAP_FILE_VERSION = 0x42;
...
const uint16 GFNT_FILE_VERSION = 0x02;
 ...
 *  Public keys
 */
const uint32 PBKEY_COMMON =     0x94A5F182;
const uint32 PBKEY_FNT =        0x8F619AB4;
const uint32 PBKEY_LNG =        0xB2A1F837;
const uint32 PBKEY_OBJ =        0x634B1F31;
const uint32 PBKEY_MDMAP =      0x1F32A27B;
const uint32 PBKEY_PBMAP =      0x4FA2B245;

const uint32 HMM_COMPOCODE=     0xd2e90517;

/*
 *  Private key or Magic Number
 */
#define PRIVATE_KEY 0xA7F9E2D
...
// solid water
#define SURF_MOVE_COST_SWATER   (-128)
#define PASSMAP_SWATER          (-128)
// part water
#define SURF_MOVE_COST_PWATER   (-127)
#define PASSMAP_PWATER          (-127)

#define PASSMAP_IMPASSABLE		(-1)
#define PASSMAP_OCCUPIED		(-2)

/*
 * Battle backgrounds
 */
enum BATTLE_BACK_TYPE {
	BBTYPE_INVALID = -1,
    BBTYPE_BARREN = 0,
    BBTYPE_BEACH,
    BBTYPE_BRICK,
...
/*
 * Battle background skies
 */
enum BATTLE_SKY_TYPE {
	BSTYPE_INVALID = -1,
    BSTYPE_BARREN_MOUNT = 0,
    BSTYPE_BEACH_SEA,
    BSTYPE_BRICK_CITY,
    BSTYPE_DIRT_FOREST,
    BSTYPE_DIRT_MOUNT,
    BSTYPE_DIRT_SEA,
    BSTYPE_GOLDEN_SAND_MOUNT,
    BSTYPE_GOLDEN_SAND_SEA,
    BSTYPE_GRASS_FOREST,
    BSTYPE_GRASS_SEA,
    BSTYPE_LAVA_MOUNT,
    BSTYPE_SHIP,
...
enum HERO_TYPE_MASK {
    ...
    HTM_VARIAG      = 0x40,
    HTM_GOOD        = HTM_KNIGHT | HTM_WIZARD | HTM_SORCERESS | HERO_TYPE_VARIAG,
    HTM_EVIL        = HTM_BARBARIAN | HTM_WARLOCK | HTM_NECROMANCER,
    HTM_MIGHT       = HTM_KNIGHT | HTM_BARBARIAN | HTM_VARIAG,
...
struct iMineralSet {
    ...
    inline void operator -= (const iMineralSet& ms) { for (uint32 xx=0; xx<MINERAL_COUNT; ++xx) quant[xx] -= ms.quant[xx]; }
	friend iMineralSet operator-(const iMineralSet &ms);	
};
...
enum SPEC_HERO_FLAGS {
    ...
    SHF_SEAFARING       // additional 40% water moves
};
...
public:
	...
	iArtifactList() {}
	iArtifactList(iArtifactList& other) { for(uint32 xx=0; xx<other.m_Items.GetSize(); xx++) { iItem n = other.m_Items[xx]; m_Items.Add(n); } }
	...
	inline void RemoveAt(uint32 idx) { m_Items.RemoveAt(idx); }
	//inline void operator=(const iArtifactList& other) { m_Items = other.m_Items; }
...
enum FURTHER_SKILLS {
    ...
    FSK_SPELLDMG,       // [Combat]     Adds 'n' percent to all spell dmg
    FSK_FEAR,           // [Combat]     Substracts 'n' from enemy morale
    FSK_SEAFARING,      // [Overland]   Hero’s sea movement range is increased by 'n'%    
    FSK_COUNT
};
//////////////////////////////////////////////////////////////////////////
class iFurtSkills
...
struct iReqItem {
    iReqItem(REQ_ITEM_TYPE type, sint32 fparam, sint32 sparam)
    : m_type(type), m_fParam(fparam), m_sParam(sparam) {}

    REQ_ITEM_TYPE       m_type;
    sint32              m_fParam;
    sint32              m_sParam;
};

typedef iSimpleArray<iReqItem> iReqsCtr;
...
extern const iPoint SHIP1_FLAG1_ANCHOR[8];
extern const iPoint SHIP1_FLAG2_ANCHOR[8];
extern const iPoint SHIP2_FLAG1_ANCHOR[8];
extern const iPoint SHIP2_FLAG2_ANCHOR[8];
...
{
	VC_INVALID = -1, // no victory achieved
	VC_DEFEAT_ALL_ENEMIES = 0, // default
	VC_DEFEAT_HERO,	
	VC_CAPTURE_CASTLE,	
	VC_SURVIVE,	
	VC_RESOURCE,
...
struct iVictoryCondition {
	iVictoryCondition(): type(VC_DEFEAT_ALL_ENEMIES), time(0) {}
	VICTORY_CONDITION_TYPE type;
	uint32				   fparam;
	uint32				   sparam;
	uint32				   time; // 0 means infinite time
};

enum LOSE_CONDITION_TYPE
{
	LC_INVALID = -1, // no lose
	LC_DEFAULT = 0, // no special losing condition
	LC_LOSE_HERO,
	LC_LOSE_CASTLE,
	LC_COUNT,
	LC_OTHER_WON = 3, //special case for message "others have accomplished victory condition"
	LC_INTIME = 4 //special case for message "you didn't finish in time"
};

struct iLoseCondition {
	iLoseCondition(): type(LC_DEFAULT) {}
	LOSE_CONDITION_TYPE	   type;
	uint32				   fparam;		
	uint32				   sparam;		
};


/* global map properites */
#define GMP_NO_SPEC_WEEKS		(1 << 0)
#define GMP_ENABLE_SHIPS		(1 << 1)

// map game mode
#define GMP_GMODE_ADVENTURE     (1 << 2)
#define GMP_GMODE_ACTION        (1 << 3)
#define GMP_GMODE_MULTIPLAYER   (1 << 4)
#define GMP_GMODE_TUTORIAL      (1 << 5)

#define GMP_GMODE_ALL (GMP_GMODE_ADVENTURE | GMP_GMODE_ACTION  | GMP_GMODE_MULTIPLAYER)

// consts
const sint32 RAND_VAL = -1;

#endif //PH_COMMON_COMMON_H_
