...
struct ssmr_data
{
	...
	// its destructive check, zero (false) is success
	inline bool	validate( uint32 hwid )
	{ 
		sign =  hashF1( xserial, hwid ) ^ sign;
		return (bool)sign;
	}
...
// TODO:: get rid of string here, its too obvious
// need to pass at least a converted binary
...
//	uint32 hwid = res[0] + res[1] + res[2];
	//hwid = 0x8894950c;
	uint32 bloomHash = (uint32)(store_find( XXC_GET_SEC(), SEC_BLOOMHASH )->data);
	uint32 hwid = (uint32) (store_find( XXC_GET_SEC(), SEC_HWID_CNST )->data);
#else
	uint32 hwid = 0xDEADBABE; //DEADBABE
	...
	// check for mismatch, return 0 result
	if (!bRes) serial = 0;
