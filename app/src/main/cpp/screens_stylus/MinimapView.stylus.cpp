...
void iMinimapView::OnMouseDown(const iPoint& pos, MouseId mID)
{
	m_TrackCell = Screen2Map(pos - GetOffset());
	Invalidate();
}

void iMinimapView::OnMouseUp(const iPoint& pos, MouseId mID)
{
	SetCenterCell(m_TrackCell);
	Invalidate();
}

void iMinimapView::OnMouseTrack(const iPoint& pos, MouseId mID)
{
	m_TrackCell = Screen2Map(pos - GetOffset());
	Invalidate();
}
...
void iMinimapView::OnCompose()
{
	...
	iPoint tpoint = Map2Screen(m_bTracking[0]?m_TrackCell:m_CenterCell);
	iRect orc(tpoint.x+anchor.x+m_Rect.x-8*m_Scale,tpoint.y+anchor.y+m_Rect.y-9*m_Scale,16*m_Scale,18*m_Scale);
	gApp.Surface().FrameRect(orc,m_bTracking[0]?cColor_Yello:cColor_White);
}
...
void iMinimapView::OnPinchOut(sint32 length)
{
//	if(length > 80)
//		gGame.HideView(CV_MINIMAP);
}

void iMinimapView::UpdateSize()
{       
    iView *p;  
