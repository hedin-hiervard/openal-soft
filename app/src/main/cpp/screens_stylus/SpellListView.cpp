...
//////////////////////////////////////////////////////////////////////////
const uint32 SPLV_SPELL_SET_MASKS[SSM_COUNT - 1] = {
	MST_MASK_ALL,
	MST_MASK_AIR,
	MST_MASK_EARTH,
	MST_MASK_FIRE,
...
public:
	...
	void UpdateSpellList(uint32 typeMask, SPELL_SET_MODES setMode)
	{
		...
		if (setMode == SSM_FAVORITES) {
			...
			uint32 spmask = SPLV_SPELL_SET_MASKS[setMode];
...
private:
	...
	void ComposeLBItem(uint32 iIdx, bool bSel, const iRect& irc)
	{
		...
		text.Addf(_T(" (%d)"), cost);
		iTextComposer::FontConfig fc(FS_SMALL,MSCH_COLORS[school][0]);
...
void iSpellListView::OnCompose() 
{
	...
	rc.DeflateRect(m_pSchoolSwitch->GetRect().x2() + 5, 0, 0, 0);
	...
	if (m_curSel != MSP_INVALID) {
		// Name
		gTextComposer.TextOut(iTextComposer::FontConfig(FS_MEDIUM,MSCH_COLORS[SPELL_DESCRIPTORS[m_curSel].school][0]), gApp.Surface(),iPoint(rc.x,rc.y),gTextMgr[TRID_SPNAME_MAGICARROW+m_curSel],rc, AlignTop);
		rc.DeflateRect(0,13,0,0);
		// Level
		gTextComposer.TextOut(iTextComposer::FontConfig(FS_SMALL,MSCH_COLORS[SPELL_DESCRIPTORS[m_curSel].school][0]), gApp.Surface(),iPoint(rc.x,rc.y),gTextMgr[TRID_LEVEL] + iFormat(_T(": %d"), SPELL_DESCRIPTORS[m_curSel].level + 1),rc, AlignTop);
		...
		gTextComposer.TextOut(iTextComposer::FontConfig(FS_SMALL,MSCH_COLORS[school][0]), gApp.Surface(),iPoint(rc.x,rc.y),gTextMgr[TRID_COST]+iFormat(_T(" %d"),cost),rc, AlignTop);
		rc.DeflateRect(0,20,0,0);
		// Inaccessebility reason
		if (m_pOwner->ManaPts() < cost) {
			iStringT msgStr(gTextMgr[TRID_MSG_SPELL_NOT_ENOUGH_MANA]);
			iTextComposer::FontConfig fntSmall(FS_SMALL,RGB16(255,96,0));
			...
			iTextComposer::FontConfig fntSmallDifferentColor(FS_SMALL,RGB16(255,255,0));
...
void iSpellListView::iCMDH_ControlCommand(iView* pView, CTRL_CMD_ID cmd, sint32 param)
{
	if (pView->GetUID() == m_UID) {
		if (cmd == CCI_LBSELCHANGED || cmd == CCI_LBSELECTED) {
	...
	} else if (pView->GetUID() == m_UID + 10 && cmd == CCI_BTNCLICK) {
		if (m_pOwner) m_pOwner->SetSpellSetMode((SPELL_SET_MODES)m_pSchoolSwitch->GetCurrentTab());
		UpdateSpellList();
	} else if (pView->GetUID() == m_UID + 20 && cmd == CCI_BTNCLICK) {
